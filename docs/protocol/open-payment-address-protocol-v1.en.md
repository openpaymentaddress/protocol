...
Protocol version 1 · URL identity revision · 20 July 2026

> **Status:** normative replacement specification. It deliberately replaces the
> earlier `label@domain` and site-association OPAP/1 profile. The repository's
> schemas, fixtures, runtime, CLI, and Browser Payer implement this revision.
> See the [requirements and
> migration record](opap-v1-url-identity-requirements.md).
...
2. Its path may identify a product, invoice, donation, or other resource.
...
the same economic recipient. Existing OPAP/1 handler semantics for SEPA and
ERC-20 remain applicable: each executable handler names a settlement currency;
an asset is identified by its network and asset identifier, never by symbol
alone. The first supported handler is the recipient preference.
...
A split remains one fixed, atomic payment instruction. It MUST have one
settlement network and one asset, fixed recipients, positive integer shares, and
the existing OPAP rounding and contract-verification guarantees. Nested splits
are forbidden.
...
A WordPress or WooCommerce plugin can implement this route and return a record
derived from a product, donation, or shop configuration. A static site may emit
the individual record files at build time. A host unable to expose the root
well-known path cannot publish OPAP Records for that origin.
...
This revision removes all of the following earlier OPAP/1 concepts:

- `label@domain` identifiers and bare-domain shorthand;
- record paths of the form `/.well-known/open-payment/<label>.json`;
- per-record DNS SHA-256 TXT hashes;
- `_site.json`, `_site._opap`, and page-association matching;
- input precedence between tagged OPIDs, page URLs, and record URLs.

An old resolver MUST reject a URL-identity record rather than reinterpret it. A
new resolver MUST reject old tagged records unless an explicitly separate legacy
compatibility module is enabled. Schemas, generated validators, conformance
vectors, CLI commands, runtime packages, Browser Payer flows, and publisher
operations require a coordinated migration before this specification can be
